...
if isinstance(raw, dict) and "metrics" in raw:
    for entry in raw.get("metrics", []):
        ...
        for metric, values in _extract_prom_results(entry):
            ...
            cpu = _maybe_int(labels.get("cpu")) if labels.get("cpu") is not None else None
...
# Always emit LLM summary file if there are no spikes
if not spike_explanations:
    msg = f"No CPU spikes detected (util_roll_z >= {Z_THRESH}) or insufficient CPU metrics."
    print("\n=== LLM Summary ===")
    print(msg)
    with open(f"{OUTPUT_DIR}/llm_summary.txt", "w") as f:
        f.write(msg)
